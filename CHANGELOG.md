...
`allium monitor` now checks a spec's invariants against either trace world under one command. It auto-detects the trace shape from how its rows are keyed: `period=`-keyed rows are a schedule, so arithmetic and aggregate (`sum`) invariants are evaluated over ordered periods with a numeric residual; `entity=`/`t=`-keyed rows are an event stream, so boolean, temporal (`old(...)`) and relational (quantified) invariants are evaluated over entities across time. Each invariant is dispatched to the evaluator its form needs, and anything the detected world cannot faithfully evaluate is reported under `skipped`, never guessed. `--format auto|event|schedule` forces a world; `--tol` sets the schedule tolerance (default 0.005). The JSON now carries a `format` field naming the world that ran and a `detected` flag. Arithmetic invariants were previously reachable only through a separate `monitor-schedule` command; that command stays as a deprecated alias with byte-identical output, so existing gates and scripts keep working. The trace format was never a real boundary between two tools, only a property of the trace, and the command now reflects that.
