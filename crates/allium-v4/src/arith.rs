...
/// True if `e` is a comparison (`=`, `<`, `<=`, `>`, `>=`, `<>`).
fn is_comparison(e: &Expr) -> bool {
    matches!(
        e,
...
/// True if `e` references `old(X)` for any state named in `modified` (so its post value depends on its
/// pre value — an ADJUST, not an OVERWRITE).
fn refs_old_on(e: &Expr, modified: &HashSet<String>) -> bool {
    match e {
        Expr::Unary { op: UnOp::Old, e } => crate::analyse::mentions_any(e, modified),
        Expr::Unary { e, .. } => refs_old_on(e, modified),
        Expr::Binary { lhs, rhs, .. } => refs_old_on(lhs, modified) || refs_old_on(rhs, modified),
        Expr::App { head, args } => refs_old_on(head, modified) || args.iter().any(|a| refs_old_on(a, modified)),
        Expr::Cond { cond, then_, els } => refs_old_on(cond, modified) || refs_old_on(then_, modified) || refs_old_on(els, modified),
        _ => false,
    }
}

/// Relational (two-entity) ARITHMETIC ordering preservation. For a 2-entity invariant `every a :: every b
/// :: G(a,b) implies A(a,b)` with linear G and a single linear A (e.g. `key(a)=key(b) and version(a) >
/// version(b) implies offset(a) >= offset(b)`), an OVERWRITE action on one entity can create an
/// out-of-order pair against another. Because the effect overwrites (does not read the pre value of the
/// modified states), the pre relationship is irrelevant, so no disjunctive pre-reasoning is needed: the
/// violation `G(post) ∧ ¬A(post)` is a clean conjunction. The unchanged partner `_f` is framed by the
/// spec's unconditional single-entity bounds. ADJUST actions (reading `old` on a modified state) are
/// skipped — those need the disjunctive pre and are out of this slice. Sound: any nonlinear term skips.
pub fn relational_arith_preservation(module: &Module, src: &str) -> Vec<Diagnostic> {
    let mut out = Vec::new();
    for d in &module.decls {
        let mut st: HashMap<String, String> = HashMap::new();
        for it in &d.items {
        ...
        // Unconditional single-entity linear invariants (`every x :: offset(x) >= 0`), reduced to `_e`
        // form — these frame the partner entity `_f` (they hold for every entity).
        let mut single_uncond: Vec<Expr> = Vec::new();
        // Relational ordering invariants: (name, va, vb, G-conjuncts, A).
        let mut rinvs: Vec<(String, String, String, Vec<Expr>, Expr)> = Vec::new();
        for it in d.items.iter().filter(|it| it.kind == ItemKind::Invariant) {
            let (name, body) = match (&it.name, it.body) {
                (Some(n), Some(b)) => (n.clone(), b),
                _ => continue,
            };
            let raw = parse_predicate(body.slice(src)).0;
            if let Some((vars, qf)) = crate::analyse::universal_body(&raw) {
                if vars.len() == 2 {
                    if let Expr::Binary { op: BinOp::Implies, lhs, rhs } = &qf {
                        if is_comparison(rhs) {
                            rinvs.push((name, vars[0].clone(), vars[1].clone(), split_and(lhs), (**rhs).clone()));
                        }
                    }
                    continue;
                }
            }
            // Single-entity unconditional linear (no implication, no sum): a partner frame.
            if let Some(e) = arith_reduce(&raw) {
                if !matches!(e, Expr::Binary { op: BinOp::Implies, .. }) && !ground(&e, &st).1 && !ground(&e, &st).0.is_empty() {
                    single_uncond.push(e);
                }
            }
        }
        if rinvs.is_empty() {
        ...
        for it in d.items.iter().filter(|it| it.kind == ItemKind::Action) {
            ...
            let mut ev = HashSet::new();
            crate::analyse::collect_entity_vars(&ensures_raw, &mut ev);
            if let Some(g) = it.requires.map(|sp| parse_predicate(sp.slice(src)).0) {
                crate::analyse::collect_entity_vars(&g, &mut ev);
            }
            if ev.len() != 1 {
                continue;
            ...
            // OVERWRITE only: skip if the effect reads `old` on a modified state.
            if refs_old_on(&ensures, &modified_num) {
                continue;
            }
            let mut st2 = st.clone();
            ...
            let effect = crate::analyse::to_post(&ensures, &modified_num, false);
            let (effect_cons, en) = ground(&effect, &st2);
            if en {
                continue;
            }
            // Partner frame: single-entity unconditional bounds instantiated at `_f`.
            let f_map: HashMap<String, String> = [("_e".to_string(), "_f".to_string())].into();
            let mut f_pre: Vec<Con> = Vec::new();
            for inv in &single_uncond {
                let (c, n) = ground(&crate::analyse::rename_vars(inv, &f_map), &st2);
                if !n {
                    f_pre.extend(c);
                }
            }

            for (rname, va, vb, g, a) in &rinvs {
                // The action can only disturb this ordering if it modifies one of the invariant's states;
                // otherwise the pair is unchanged and the invariant (assumed to hold pre) still holds.
                let touches = g.iter().any(|c| crate::analyse::mentions_any(c, &modified_num))
                    || crate::analyse::mentions_any(a, &modified_num);
                if !touches {
                    continue;
                }
                for map in [
                    HashMap::from([(va.clone(), "_e".to_string()), (vb.clone(), "_f".to_string())]),
                    HashMap::from([(va.clone(), "_f".to_string()), (vb.clone(), "_e".to_string())]),
                ] {
                    // Instantiate G and A, resolve entity (in)equalities, prime the modified subject `_e`.
                    let g_inst: Vec<Expr> = g
                        .iter()
                        .map(|c| crate::analyse::to_post(&crate::analyse::resolve_entity_eq(&crate::analyse::rename_vars(c, &map)), &modified_num, false))
                        .collect();
                    if g_inst.iter().any(|c| matches!(c, Expr::Name(n) if n == "false")) {
                        continue; // an entity-equality made the guard impossible for this pairing
                    }
                    let a_inst = crate::analyse::to_post(&crate::analyse::resolve_entity_eq(&crate::analyse::rename_vars(a, &map)), &modified_num, false);
                    let mut gcons = Vec::new();
                    let mut gok = true;
                    for c in &g_inst {
                        if matches!(c, Expr::Name(n) if n == "true") {
                            continue;
                        }
                        let (cc, n) = ground(c, &st2);
                        if n {
                            gok = false;
                            break;
                        }
                    ...
                    let (acons, an) = ground(&a_inst, &st2);
                    if an || acons.is_empty() {
                        continue;
                    }
                    let mut broke = false;
                    'search: for ac in &acons {
                        for neg in negate_con(ac) {
                            let mut q: Vec<Con> = effect_cons.clone();
                            q.extend(f_pre.clone());
                            q.extend(gcons.clone());
                            q.push(neg);
                    ...
                    if broke {
                        out.push(Diagnostic::warning(
                            it.span,
                            format!("action `{aname}` in `{}` can break relational ordering invariant `{rname}`: overwriting one entity can create a pair that violates the ordering against another entity. Ensure the new value keeps the order, or guard the action.", d.name),
                        ));
                        break; // one report per (action, invariant)
                    }
                }
            }
        }
...
#[cfg(test)]
mod tests {
    use super::{aggregate_preservation, arithmetic, enum_guarded_preservation, relational_arith_preservation};
    ...
    #[test]
    fn relational_arith_ordering_preservation() {
        let rap = |src: &str| -> Vec<String> {
            let m = parse(src).module;
            relational_arith_preservation(&m, src).into_iter().map(|d| d.message).collect()
        };
        let hdr = "-- allium: 4\ncomponent Shard\n  entity S\n  observable state key(S) : Number\n  observable state version(S) : Number\n  observable state ofs(S) : Number\n  invariant ordered means every a :: every b :: key(a) = key(b) and version(a) > version(b) implies ofs(a) >= ofs(b)\n";
        // Overwriting version/offset without preserving the order breaks it.
        let bad = format!("{hdr}  action record\n    ensures version(s) = 100 and ofs(s) = 0\nend\n");
        assert!(any(&rap(&bad), "`record` in `Shard` can break relational ordering invariant `ordered`"), "{:#?}", rap(&bad));
        // Resetting to the minimum version (with version/ofs >= 0) cannot create an out-of-order pair.
        let safe = format!("-- allium: 4\ncomponent Shard\n  entity S\n  observable state key(S) : Number\n  observable state version(S) : Number\n  observable state ofs(S) : Number\n  invariant vnn means version(s) >= 0\n  invariant onn means ofs(s) >= 0\n  invariant ordered means every a :: every b :: key(a) = key(b) and version(a) > version(b) implies ofs(a) >= ofs(b)\n  action reset\n    ensures version(s) = 0 and ofs(s) = 0\nend\n");
        assert!(!any(&rap(&safe), "can break relational ordering"), "{:#?}", rap(&safe));
        // An ADJUST action (reads `old` on a modified state) is out of this slice — not flagged.
        let adjust = format!("{hdr}  action bump\n    ensures version(s) = old(version(s)) + 1 and ofs(s) = old(ofs(s)) + 1\nend\n");
        assert!(!any(&rap(&adjust), "can break relational ordering"), "{:#?}", rap(&adjust));
        // An action that touches unrelated state does not disturb the ordering.
        let unrelated = format!("{hdr}  observable state note(S) : Number\n  action annotate\n    ensures note(s) = 1\nend\n");
        assert!(!any(&rap(&unrelated), "can break relational ordering"), "{:#?}", rap(&unrelated));
    }
